...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            b, n, c, h, w = self.lq.size()

            tile = min(180, h, w)
            tile_overlap = 16
            sf = 4
            # output = []
            stride = tile - tile_overlap  # 48
            h_idx_list = list(range(0, h - tile, stride)) + [h - tile]
            w_idx_list = list(range(0, w - tile, stride)) + [w - tile]
            E1 = torch.zeros(b, n, c, h * sf, w * sf).type_as(self.lq)
            W1 = torch.zeros_like(E1)
            # for i in range(n):
            for h_idx in h_idx_list:
                for w_idx in w_idx_list:
                    in_patch = self.lq[..., h_idx:h_idx + tile, w_idx:w_idx + tile]
                    out_patch1 = self.net_g(in_patch)
            ...
            #     overlap = 16   # TODO
            #     self.output = forward_crop(self.lq, self.net_g, lq_size=64, overlap=16)
            # else:
            #     # if self.opt['train'].get('flow_opt'):
            ...
            #     self.output = self.net_g(self.lq)
        if flip_seq:
            output_1 = self.output[:, :t, :, :, :]
            output_2 = self.output[:, t:, :, :, :].flip(1)
            self.output = 0.5 * (output_1 + output_2)

        if self.center_frame_only:
            self.output = self.output[:, t // 2, :, :, :]
